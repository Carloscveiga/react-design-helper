...
interface Prop {
  ...
  label: string
  group: string
  type: 'color' | 'color-alpha' | 'range' | 'select' | 'text' | 'image' | 'size' | 'tracking' | 'font' | 'rotation'
  options?: string[]
  min?: number; max?: number; step?: number; unit?: string
  cond?: (s: CSSStyleDeclaration) => boolean
  get: (s: CSSStyleDeclaration, el: HTMLElement) => string
...
function RotationInput({ value, onChange }: { value: string; onChange: (v: string) => void }) {
...
function ColorAlphaInput({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  ...
  const emit = (newHex: string, newAlpha: number) => {
...
function PropRow({ prop, value, origVal, onChange }: { prop: Prop; value: string; origVal: string; onChange: (v: string) => void }) {
...
export function EditHelper() {
  ...
  const [origVals, setOrigVals] = useState<Record<string, string>>({})
  ...
  const [copied, setCopied] = useState<null | 'classes' | 'content' | 'css'>(null)

  function copyText(text: string, type: 'classes' | 'content' | 'css') {
    navigator.clipboard.writeText(text)
    setCopied(type)
    setTimeout(() => setCopied(null), 1500)
  }
  const [collapsed, setCollapsed] = useState<Set<string>>(new Set(['Sizing', 'Padding', 'Margin', 'Typography', 'Background', 'Border', 'Effects']))
  ...
  const touchedMapRef = useRef<Map<number, Set<string>>>(new Map())
  ...
  function startTreeResize(e: React.MouseEvent) {
    ...
    function onMove(e: MouseEvent) { setTreeHeight(Math.max(80, Math.min(600, startH + e.clientY - startY))) }
    function onUp() { window.removeEventListener('mousemove', onMove); window.removeEventListener('mouseup', onUp) }
    window.addEventListener('mousemove', onMove)
    window.addEventListener('mouseup', onUp)
  }
  ...
  const propertiesContent = selectedNode && computedStyles && (
    <div style={{ flex: 1, overflowY: 'auto', padding: '6px 14px 16px' }}>
      ...
      {(() => {
        ...
        const cellStyle: React.CSSProperties = { flex: 1, background: '#18181b', borderRadius: 4, padding: '6px 8px', fontFamily: 'monospace', fontSize: 9, color: '#a1a1aa', whiteSpace: 'pre', overflowX: 'auto', lineHeight: 1.6 }
        return (
          <div style={{ marginBottom: 10 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
