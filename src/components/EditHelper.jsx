...
export function EditHelper() {
  ...
  function moveNode(dir) {
    ...
    if (dir === 'up') {
      ...
      while (prev && prev === wrapRef.current) prev = prev.previousElementSibling
      if (prev) parent.insertBefore(el, prev)
    } else {
      let next = el.nextElementSibling
      while (next && next === wrapRef.current) next = next.nextElementSibling
    ...
    const all = [{ id: 0, el: rootParent, depth: 0 }, ...children]
    all.forEach((n, i) => { n.hasChildren = i < all.length - 1 && all[i + 1].depth > n.depth })
